...
def clipRegion(inPath, outPath):
    ...
    gdal.Warp(outPath, inPath, dstSRS=utmPrjFile, format='GTiff', outputBounds=TAIHU_BOUNDS,
              dstNodata=65535, xRes=10, yRes=10)
...
def cloudProduct(inPath, outPath):
    ...
    redBandIndex = ds.GetRasterBand(3)
    bandR = redBandIndex.ReadAsArray()

    cloudArray = np.zeros((height, width), dtype=np.uint8)
    # 云检测阈值
    cloudArray[np.logical_and(bandR > 4, bandR != 65535)] = 1
...
def chla(inPath, taihuPath, cloudPath, outPath):
    ...
    height = ds.RasterYSize
    redBandIndex = ds.GetRasterBand(3)
    nirBandIndex = ds.GetRasterBand(4)
    bandR = redBandIndex.ReadAsArray()
...
def falseColor(inputPath,inputpath, outputDir):
    ...
    RgbComposite.Compose(inputpath, (4, 3, 2), noDataValue=65535, returnMode='GEOTIFF', outputPath=falseColorTifPath,
...
def executeSql(modelUuid,chlaTifPath):
    ...
    if len(sqlRes) > 0:
        ...
        sqlData = (image_uuid, chlaTifName, db_path, 'tif', 'chla', 'Sentinel2', 'MSI', '30.8897', '120.66946', '119.8411',
...
def main(inputPath, modelUuid):
    ...
    dependDir = globalCfg['depend_path']
    taihuPath = os.path.join(dependDir, 'taihu', '10', 'taihu_mask_utm[0].tif')
...
if __name__ == '__main__':
    ...
    #modelUuid = 'S2_chla'
    #inputPath = r"D:/jsdata/Sentinel2B_MSI_L2_202101031041_10_0209_089.tif"
